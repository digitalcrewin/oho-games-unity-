...
public class T_BluePlayer : T_Players
{
    public static new T_BluePlayer instance;
    public T_DiceManager blueRollingDice;
    ...
    {
        if (T_SocketController.instance.myColor == "blue")
        {
                    ...
                    if (T_SocketController.instance.myPreservedMoves.Count > 0)
                    {
                        if (distance != 0 && distance != 57)
                        {
                            ...
                            else if (T_SocketController.instance.myPreservedMoves.Count == 2 || T_SocketController.instance.myPreservedMoves.Count == 3)
                            {
                                ...
                                for (int i = 0; i < T_GameManager.instance.bluePlayers.Length; i++) //4
                                {
                                    int tempI = i;
                                    for (int j = 0; j < T_SocketController.instance.myPreservedMoves.Count; j++)  //2
                                    {
                                        ...
                                        {
                                            T_GameManager.instance.bluePlayers[i].DiceValue6Options1[tempJ].transform.parent.gameObject.SetActive(true);
                                            OptionMove(T_WayPointPathPrent.instance.blueWayPointPath);
                                            T_GameManager.instance.bluePlayers[i].DiceValue6Options1[tempJ].transform.parent.GetComponent<Button>().onClick.RemoveAllListeners();
                                            T_GameManager.instance.bluePlayers[i].DiceValue6Options1[tempJ].text = T_SocketController.instance.myPreservedMoves[tempJ].ToString();
                                            T_GameManager.instance.bluePlayers[i].DiceValue6Options1[tempJ].transform.parent.GetComponent<Button>().onClick.AddListener(() =>
                                            {
                                                if (T_SocketController.instance.myPreservedMoves.Count == 1)
                                                {
                                                    T_SocketController.instance.PlayerMove(currentWayPointInt, T_SocketController.instance.myPreservedMoves[0].ToString());
                                                ...
                                                T_GameManager.instance.bluePlayers[tempI].DiceValue6Options1[tempJ].transform.parent.gameObject.SetActive(false);
                                                T_GameManager.instance.bluePlayers[tempI].DiceValue6Options1[tempJ].text = "";
