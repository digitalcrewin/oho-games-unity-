...
public class R_SortCardManager : MonoBehaviour
{
    //Instance so we can access it from all scripts
    public static R_SortCardManager instance;
    ...
    private R_SortCardView selectedCard, previousCard, nextCard;  //ref to cards
    int k, childCount;                                      
    private GameObject dummyCardObj;

    public R_SortCardView SelectedCard { get => selectedCard; }
    ...
    public void SelectCard(R_SortCardView card)
    {
        ...
        selectedCard.transform.SetParent(R_SortCardManager.instance.ParentHolder.transform);
        ...
        if (selectedIndex + 1 < childCount)
        {
            ...
            nextCard = cardHolder.transform.GetChild(selectedIndex + 1).GetComponent<R_SortCardView>();
        }

        if (selectedIndex - 1 >= 0)
        {
            //set the previous card of selected card
            previousCard = cardHolder.transform.GetChild(selectedIndex - 1).GetComponent<R_SortCardView>();
        }
    ...
    public void OnCardRelease()
    {
        ...
        if (SelectedCard != null)
        {
            ...
            {
                GetDummyCard().transform.SetParent(R_SortCardManager.instance.ParentHolder.transform);
    ...
    void CheckWithNextCard()
    {
        if (nextCard != null)
        {
            if (selectedCard.transform.position.x > nextCard.transform.position.x)
            {
                ...
                {
                    nextCard = cardHolder.transform.GetChild(index + 1).GetComponent<R_SortCardView>();
                }
                else
                {
                    nextCard = null;
    ...
    void CheckWithPreviousCard()
    {
        if (previousCard != null)
        {
            if (selectedCard.transform.position.x < previousCard.transform.position.x)
            {
                ...
                if (index - 1 >= 0)
                {
                    previousCard = cardHolder.transform.GetChild(index - 1).GetComponent<R_SortCardView>();
                }
                else
                {
                    previousCard = null;
    ...
    void SpawnCard()
    {
        ...
        card.GetComponent<R_SortCardView>().SetCardImg(cardImages[k]);
    }
