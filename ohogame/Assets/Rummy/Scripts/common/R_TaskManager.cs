...
public class R_Task
{
	...
	/// upon construction.
	public R_Task(IEnumerator c, bool autoStart = true)
	{
		task = R_TaskManager.CreateTask(c);
	...
	R_TaskManager.R_TaskState task;
}

class R_TaskManager : MonoBehaviour
{
	public class R_TaskState
	{
		public bool Running {
			get {
				return running;
		...
		public R_TaskState(IEnumerator c)
		{
			coroutine = c;
		}
	...
	static R_TaskManager singleton;

	public static R_TaskState CreateTask(IEnumerator coroutine)
	{
		if(singleton == null) {
			GameObject go = new GameObject("TaskManager");
			singleton = go.AddComponent<R_TaskManager>();
		}
		return new R_TaskState(coroutine);
	}
}
