...
public class P_TableSettings : MonoBehaviour
{
    ...
    public GameObject muckhandon, muckhandoff;
    ...
    private int issound, isVociemsg, isTextmsg, isvibrations, isguesyourNexthand, isexactBetting, iscustomizedActionbtn, israise, isMuckHand;
    public void Start()
    {
            ...
            case "muckhand":
                {
                    if (isMuckHand == 1) //on
                    {
                        P_SocketController.instance.SendToggleMuck(false); //off
                        isMuckHand = 0;
                        muckhandoff.SetActive(true);
                        muckhandon.SetActive(false);
                        PlayerPrefs.SetInt("isMuckHand", isMuckHand); //0
                    }
                    else //off
                    {
                        P_SocketController.instance.SendToggleMuck(true); //on
                        isMuckHand = 1;
                        muckhandon.SetActive(true);
                        muckhandoff.SetActive(false);
                        PlayerPrefs.SetInt("isMuckHand", isMuckHand);  //1
                    }
                }
                break;
            case "voicemsg":
    ...
    void LoadResetTableData()
    {
        ...
        if(PlayerPrefs.GetInt("isMuckHand") == 1)
        {
            isMuckHand = 1;
            muckhandon.SetActive(true);
            muckhandoff.SetActive(false);
