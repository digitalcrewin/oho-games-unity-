...
public class P_PrefsManager : MonoBehaviour
{
    public static void SetData(P_PrefsKey prefsKey, string dataToAssign)
    {
        PlayerPrefs.SetString("" + prefsKey, dataToAssign);
    }

    public static P_RoomData GetRoomData()
    {
        P_RoomData roomData = new P_RoomData();

        string stringData = PlayerPrefs.GetString("" + P_PrefsKey.RoomData);
        ...
        if (!string.IsNullOrEmpty(stringData))
        {
            ...
            {
                string[] gameModes = System.Enum.GetNames(typeof(P_GameMode));
                string currentName = data["gameMode"].ToString();
                roomData.gameMode = P_GameMode.NLH;

                for (int i = 0; i < gameModes.Length; i++)
                {
                    if (currentName == gameModes[i])
                    {
                        roomData.gameMode = (P_GameMode)i;
                        break;
                    }
                }
            }
    ...
    public static void SetPlayerGameData(P_PlayerGameDetails dataToAssign)
    {
        PlayerPrefs.SetString("" + P_PrefsKey.PlayerGameData, JsonUtility.ToJson(dataToAssign));
    }

    public static void DeletePlayerData()
    {
        PlayerPrefs.DeleteKey("" + P_PrefsKey.PlayerGameData);
    }

    public static P_PlayerGameDetails GetPlayerData()
    {
        P_PlayerGameDetails playerData = new P_PlayerGameDetails();

        string prefsData = PlayerPrefs.GetString("" + P_PrefsKey.PlayerGameData, "");
        Debug.Log("Saved Data " + prefsData);
        if (!string.IsNullOrEmpty(prefsData))
        {
            ...
            playerData.userMobile = data["userMobile"].ToString();
            playerData.userEmail = data["userEmail"].ToString();
            playerData.userToken = data["userToken"].ToString();
            ...
            if (data.Keys.Contains("cashAmount") && data["cashAmount"].ToString() != null && data["cashAmount"].ToString().Length > 0)
            {
                playerData.cashAmount = data["cashAmount"].ToString();
            }
            else
            {
                playerData.cashAmount = "0";
            }
        }
        else
        {
